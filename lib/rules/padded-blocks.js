...
module.exports = {
  ...
  create(context) {
    ...
    if (typeof typeOptions === "string") {
      ...
      if (exceptOptions.includeObjects === true) {
        options.objects = { shouldHavePadding, onlyPadSliced };
      }
    }
    else {
      ...
      if (Object.prototype.hasOwnProperty.call(typeOptions, "objects")) {
        options.objects = {
          shouldHavePadding: typeOptions.objects === "always",
          onlyPadSliced: typeOptions.objects === "loose"
        };
      }
    }
    ...
    function getNodeOwnRanges(node) {
      ...
      if (!nodeOwnRanges[key]) {
        ...
        if (node.body) {
          node.body.forEach(function (childNode) {
            if (childNode.type === "SwitchStatement" || childNode.type === "StaticBlock" ||
                ...
                childNode.type === "ObjectPattern" || childNode.type === "ObjectExpression"
               ) {
        ...
        if (node.properties) {
          node.properties.forEach(function (childNode) {
            let valueStart
            if (childNode.value) { // Normal property
              valueStart = childNode.value
            }
            if (childNode.argument) { // Spread operator
              valueStart = childNode.argument
            }
            if(valueStart) {
              nodeOwnRanges[key].push([ start, valueStart.range[0] - 1 ]);
              start = childNode.range[1] + 1;
            }
          });
        }
    ...
    function requirePaddingFor(node, isBlockSliced) {
      switch (node.type) {
        ...
        case "ObjectExpression":
          return options.objects.shouldHavePadding || ( options.objects.onlyPadSliced && isBlockSliced );
    ...
    function shouldOnlyPadSliced(node) {
      switch (node.type) {
        ...
        case "ObjectPattern":
        case "ObjectExpression":
          return options.objects.onlyPadSliced;
    ...
    if (Object.prototype.hasOwnProperty.call(options, "objects")) {
      ...
      rule.ObjectPattern = rule.ObjectExpression;
    }
